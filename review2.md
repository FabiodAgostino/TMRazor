...
| Feature | Stato | Note |
|---------|-------|------|
| **Encryption Patch** | ✅ Implementato | Tramite `GeneralViewModel` e flag in `InstallLibrary` |
| **Multi-client injection** | ✅ Implementato | `AllowMultiClient` tramite `UOModService` |
| **Party System** | ❌ Non migrato | Nessun handler per pacchetti party (0x6F party invite, party member updates) |
| **Secure Trade completo** | ⚠️ Stub | `SecureTradeService` e `SecureTradePage` sono placeholder; l'handler 0x6F è parziale |
| **IsCasting tracking** | ❌ Mancante | Il flag `WorldService.IsCasting` non viene mai impostato da nessun handler |
| **Auto Carver** | ❌ Non migrato | Configurazione in `UserProfile` ma nessun servizio |
| **Bone Cutter** | ❌ Non migrato | Idem |
| **Auto Remount** | ❌ Non migrato | Idem |

### 5.2 Filtri e Display (Media Priorità)

| Feature | Stato | Note |
|---------|-------|------|
| **Light Filter — max brightness packet** | ⚠️ Parziale | Blocca 0x4E/0x4F ma non invia un pacchetto di massima luminosità al client. In TMRazor originale veniva inviato un pacchetto 0x4F/0x4E con valore=0 per forzare full light |
...
| **Block Trade Request** | ❌ Non migrato | Flag presente in profilo, nessun filtro su 0x6F |
| **Block Party Invite** | ❌ Non migrato | Flag presente in profilo, nessun filtro |
| **Highlight/Colorize Flags** | ❌ Non migrato | `HighlightFlags`, `ColorizeFlags` presenti ma non implementati |
| **Auto Screenshot on Death** | ✅ Implementato | Implementato in `ScreenCaptureService` e `WorldPacketHandler` |
...
| **Gump List Manager** | ✅ Implementato | UI e ViewModel dedicati |
...
| **Skill Gain History** | ✅ Implementato | Grafico a barre e lista gain in `SkillsPage` |
...
| Comando | Importanza |
|---------|-----------|
| `setoption`, variabili script | Alta |
| `overhead` (messaggi sopra entità) | Alta |
| `poplist`, `pushlist`, `removelist` | Media |
| `clearsysmsg`, `clearjournal` | Media |
| `setability` (primary/secondary ability) | Media |
| `interrupt` | Media |
| `timername`, `timer` | Bassa |
| `random` | Bassa |
...
| Completezza funzionale | 8/10 | Sprint 12 completato, ~85% feature parity |
...
| UI/UX | 8/10 | Dashboard, Skills e Gump List integrate in MainWindow |
...
### Sprint 12 — Polish & Deploy (Completato)
1. **Encryption patch + multi-client**: Implementato tramite `GeneralViewModel` (flag 0x08) e `UOModService`.
2. **Screen capture reale (PrintWindow Win32)**: Implementato in `ScreenCaptureService.cs` con hook `user32.dll`.
3. **Gump List Manager UI**: Implementato in `GumpListPage.xaml` e `GumpListViewModel.cs`.
4. **Skill Gain History grafico**: Implementato in `SkillsPage.xaml` con DataGrid e Bar Chart visuale.
